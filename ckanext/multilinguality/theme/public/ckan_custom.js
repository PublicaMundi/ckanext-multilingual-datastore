var CKAN = {};
var COL_NAME = 'stop_name';
var LANGUAGE = 'en';
var RES_ID = 'a375c051-11e1-4d65-b9a2-10e923355ecf';
...
(function(my) {
  ...
  my.Client.prototype._ajax = function(options, cb) {
    ...
    if (this.apiKey) {
      ...
      //options.headers['Authorization'] = 'a3bcd433-0f14-4ceb-aa98-1a0332a2526a';
    }
...
my.Client.prototype.datastoreCreate = function(id, queryObj, cb) {
    var actualQuery = my._normalizeQuery(queryObj);
    //console.log(queryObj);
    var l = {};
    l[id] = '';
    actualQuery['method'] = 'insert';
    //actualQuery['allow_update_with_id'] = true;
    actualQuery['force'] = true;
    actualQuery['records'] = [l];
    //actualQuery['records'] = [{'_id':3, 'stop_name_el': 'LAAA'}];
     
    //[{'_id':3, 'stop_name':'LELE'}];
    this.action('datastore_create', actualQuery, function(err, results) {
      if (err) {
        cb(err);
        console.log(err);
        return;
  ...
  my.Client.prototype.datastoreUpdate = function(queryObj, cb) {
    ...
    actualQuery['allow_update_with_id'] = true;
    actualQuery['force'] = true;
    actualQuery['records'] = queryObj.updates;
    //actualQuery['records'] = [{'_id':3, 'stop_name_el': 'LAAA'}];
    //records = actualQuery.records;
    var records = [];
    var extra = COL_NAME + '-' + LANGUAGE;
    ...
    /*that.update = function(data) {
        var updateUrl = that.endpoint + '/3/action/datastore_upsert';
        var req_ops = {
            'method' : 'update',
            'allow_update_with_id' : true,
            'resource_id' : 'df041497-f42b-4158-b4ce-097fafe7a190',
            'records' : [{
                '_id':'1',
                'stop_name': 'LALA'
            }]
...
recline.Backend.Ckan1 = recline.Backend.Ckan1 || {};
(function(my) {
  my.__type__ = 'ckan1';
  ...
  my.save = function(queryObj, dataset, lala) {
      console.log('my.save');
      //console.log(lala);
      //console.log(queryObj);
      ...
      queryObj.resource_id = dataset.id;
     wrapper.datastoreUpdate(queryObj,function(err, out){
     //wrapper.datastoreCreate('stop_name-el', queryObj,function(err, out){
     if (err) {
         console.log(err);
     }
     console.log(out);
