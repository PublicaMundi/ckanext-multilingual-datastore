...
def create_table(context, data_dict):
    ...
    datastore_fields = [
        {'id': '_id', 'type': 'serial primary key', 'state':'default'},
...
def create_view(context, data_dict):
    ...
    #fields = datastore_fields + supplied_fields + extra_fields
    view_name = data_dict['resource_id']+'-'+data_dict.get('language')
    data_dict.update({'resource_id': view_name})
    lst = []
    for tf in supplied_fields:
        if tf in trans_fields:
            lst.append('"{0}".{1}'.format(trans_table, tf))
        else:
            lst.append('"{0}".{1}'.format(orig_table, tf))
    sql_fields = u", ".join(lst)
    pprint.pprint(sql_fields)
    #sql_fields = u", ".join([u'"{0}" {1}'.format(
    #    f['id'], f['type']) for f in fields])

    sql_string = u'''CREATE VIEW "{0}" AS
                SELECT {1}
                FROM "{2}"
                INNER JOIN "{3}"
                ON "{2}"._id = "{3}"._id;'''.format(
        view_name,
        sql_fields,
        orig_table,
        trans_table,
...
def alter_view(context, data_dict):
    '''Alter view from combination of fields.'''
...
def get_view(context, data_dict):
    '''
    '''
    engine = _get_engine(data_dict)
    ...
    #_cache_types(context)
    view_name = data_dict.get('resource_id')+'-'+data_dict.get('language')
    #data_dict.update({'resource_id':view_name})
    ...
    try:
        ...
        if engine.dialect.has_table(context['connection'],view_name):
            print 'HAS TABLE'
            #result = context['connection'].execute(
                #u'SELECT * FROM _table_metadata WHERE name = %s',
                #view_name
                #u'SELECT * FROM "{0}"'.format(view_name)
            #).fetchone()
            #print result
        else:
            print 'DOESNT HAVE TABLE'
            print 'creating view'
            #create_view(context, data_dict)
            create_alias(context, data_dict)

        #alter_view(context, data_dict)
        #pass
        #alter_view(context, data_dict)
...
def search_data(context, data_dict):
    ...
    #sql_string2 = u'''CREATE VIEW TRANS AS
    #                SELECT "_id", "stop_id", "stop_name"
    #                FROM "{resource}"'''.format(
    #                        resource=data_dict['resource_id'])

    #sql_string3 = u'''SELECT *, count(*) over() AS "_full_count" {rank}  FROM TRANS {ts_query} {where} {sort} LIMIT {limit} OFFSET {offset}'''.format(
    #        rank=rank_column,
    #        ts_query=ts_query,
    #        where='{where}',
    #        sort=sort,
    ...
    #results = context['connection'].execute(
    #    sql_string2.format(where=where_clause), [where_values])
